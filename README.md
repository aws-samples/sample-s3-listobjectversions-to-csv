...
* **Offline filtering and analysis**: Create multiple manifests from one export, without repeated API calls. [Analyze size or storage class distribution with the one-line examples below!](https://github.com/aws-samples/sample-s3-listobjectversions-to-csv/tree/main?tab=readme-ov-file#analyze-total-and-average-object-sizes-excluding-delete-markers)
...
#### Filter for only noncurrent verions, excluding delete markers
```
awk -F',' 'NR==1 || ($4=="False" && $5!="True")' s3_object_versions.csv > ./noncurrent_versions.csv
```

#### Filter for only objects <128 KiB, excluding delete markers
```
awk -F',' 'NR==1 || ($6<131072&& $5!="True")' ./s3_object_versions.csv > ./small_objects.csv
```
...
#### Analyze total and average object sizes (excluding delete markers)
...
```
*Tip: This can be run against a filtered output of current or noncurrent object versions.*
...
#### Analyze storage class usage (excluding delete markers)
```
...
```
*Tip: This can be run against a filtered output of < or >= 128 KiB objects.*
